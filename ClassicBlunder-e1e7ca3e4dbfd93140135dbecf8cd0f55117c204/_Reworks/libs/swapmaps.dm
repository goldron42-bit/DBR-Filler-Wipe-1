...
swapmap
		...
		for(z=z1,z<=z2,++z)
			...
			for(y=y1,y<=y2,++y)
				...
				for(x=x1,x<=x2,++x)
					...
					var/typing
					S["type"] >> typing
					var/turf/T = new typing(locate(x,y,z))
	...
	proc/Save()
		if(id==src) return 0
		var/savefile/S=mode?(new):new("Maps/map_[id].sav")
		S << src
		while(locked) sleep(1)
		if(mode)
			fdel("Maps/map_[id].txt")
			S.ExportText("Maps/","map_[id].txt")
		return 1
	...
	proc/CenterTile(z=z1)
		return locate(x1+(x2-x1)/2, y1+(y2-y1)/2, z)
	/*
...
proc/SwapMaps_Find(id)
	InitializeSwapMaps()
	if(length(swapmaps_byname) <= 0) return null
...
proc/SwapMaps_Load(id)
	...
	if(!M)
		var/savefile/S
		var/text=0
		if(swapmaps_mode==SWAPMAPS_TEXT && fexists("Maps/map_[id].txt"))
			text=1
		else if(fexists("Maps/map_[id].sav"))
			S=new("Maps/map_[id].sav")
		else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("Maps/map_[id].txt"))
			text=1
		else return	// no file found
		if(text)
			S=new
			S.ImportText("Maps/",file("map_[id].txt"))
		S >> M
...
proc/SwapMaps_CreateFromTemplate(template_id, boxIt = TRUE)
	...
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("Maps/map_[template_id].txt"))
		text=1
	else if(fexists("Maps/map_[template_id].sav"))
		S=new("Maps/map_[template_id].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("Maps/map_[template_id].txt"))
		text=1
	else
	...
	if(text)
		S=new
		S.ImportText("Maps/",file("map_[template_id].txt"))
	/*
...
proc/SwapMaps_LoadChunk(chunk_id,turf/locorner)
	...
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("Maps/map_[chunk_id].txt"))
		text=1
	else if(fexists("Maps/map_[chunk_id].sav"))
		S=new("Maps/map_[chunk_id].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("Maps/map_[chunk_id].txt"))
		text=1
	else
	...
	if(text)
		S=new
		S.ImportText("Maps/",file("map_[chunk_id].txt"))
	/*
		This hacky workaround is needed because S >> M will create a brand new
		M to fill with data. There's no way to control the Read() process
		properly otherwise. The //.0 path should always match the map, however.
